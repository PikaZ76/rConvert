...
def main():
    ...
    for i_reg_idx in range(4):
        ...
        result_list = []
        for col_i in range(idx.shape[1]):
            col_in = idx[:, col_i]  # 这一列
            col_out = lag_normalize(col_in, lag_month)
            result_list.append(col_out)

        # 每列返回 length = (n_t_orig - lag_month)
        idx_dn = np.column_stack(result_list)  # shape => ((n_t_orig - lag_month), n_sec)

        # R里紧接着: n.t <- n.t - lag.month
        ...
        # rows_list里每个元素长度 = (n_t_orig - lag_month)
        # 拼接成 2D, shape => (n_issue, n_t_orig - lag_month)
